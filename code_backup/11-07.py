...
# =============== Newspaper Detection ===============
def detect_newspaper_name_with_debug(image_path):
    try:
        ...
        # Optional: Display OCR text for debugging
        st.write("📝 Detected Text:", combined_text[:500])  # Limit output

        return detected_name

    except Exception as e:
        st.error(f"❌ Newspaper detection error: {e}")
...
# =============== Extract Function ===============
def extract_ads(column_buffers, author_name):
    prompt = f"""
...
def process_image(image_path):
    author = detect_newspaper_name_with_debug(image_path)
    columns = segment_image(image_path)
    if not columns:
        return []
    ads = extract_ads(columns, author)
    if not ads:
        return []
